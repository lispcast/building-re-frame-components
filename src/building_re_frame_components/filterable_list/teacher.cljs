(ns building-re-frame-components.filterable-list.teacher
...
(def vehicles (sort-by :name [{:name "V-Wing Fighter", :image "/img/v-wing_8d8d05aa.jpeg"} 
...
(defn ->regex [s]
  (re-pattern (apply str "(?i)" (interpose ".*" s))))

(defn filterable-list [fltr]
  (let [s (reagent/atom {:filter-string fltr})]
    (fn []
      [:div
       [:input {:type :text :value (:filter-string @s)
                :on-change #(swap! s assoc :filter-string (-> % .-target .-value))}]
       (doall
        (for [{:keys [name image]} vehicles
              :when (re-find (->regex (:filter-string @s)) name)]
          [:div {:key name
                 :style {:height 100}}
...
(when-some [el (js/document.getElementById "filterable-list--teacher")]
